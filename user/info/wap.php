...
if (isset($status['id']))
{
	...
	$l=dbresult(dbquery("SELECT COUNT(*) FROM `status_like` WHERE `id_status` = '$status[id]'"),0);if (isset($user) && $user['id']!=$ank['id'] && dbresult(dbquery("SELECT COUNT(*) FROM `status_like` WHERE `id_status` = '$status[id]' AND `id_user` = '$user[id]' LIMIT 1"),0)==0)
{	
...
if (isset($dir_osn['id'])) echo "<a href='/user/personalfiles/$ank[id]/$dir_osn[id]/'>Файлы</a> ";
...
echo "</div>";
if ($user['wall']==0){
include_once check_replace(H.'user/stena/index.php');
}
/*
========================================
The End
