    ...
    && dbresult(dbquery("SELECT COUNT(*) FROM `like_object` WHERE `id_object` = '$news[id]' AND `type` = 'news' AND `id_user` = '$user[id]'"),0) == 0)
{
  ...
  include_once check_replace(H.'sys/add/user.active.php');
}
/*------------------------------------------------------------*/
...
if (isset($list['id'])) echo '<span class="page">' . ($list['id'] ? '<a href="?id=' . $list['id'].'">&laquo; Пред.</a> ':'&laquo; Пред. ') . '</span>';

$k_1 = dbresult(dbquery("SELECT COUNT(*) FROM `news` WHERE `id` > '$news[id]'"),0)+1;
$k_2 = dbresult(dbquery("SELECT COUNT(*) FROM `news`"),0);
echo ' (' . $k_1 . ' из ' . $k_2 . ') ';

if (isset($list['id'])) echo '<span class="page">' . ($listr['id'] ? '<a href="?id=' . $listr['id'] . '">След. &raquo;</a>' : ' След. &raquo;') . '</span>';
echo '</div>';
/*----------------------alex-borisi---------------*/
