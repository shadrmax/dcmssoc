...
$k_post=dbresult(dbquery("SELECT COUNT(*) FROM `obmennik_files` WHERE `opis` like '%".mysql_real_escape_string($q_search)."%' OR `name` like '%".mysql_real_escape_string($q_search)."%'"),0);
...
$q=dbquery("SELECT * FROM `obmennik_files` WHERE `opis` like '%".mysql_real_escape_string($q_search)."%' OR `name` like '%".mysql_real_escape_string($q_search)."%' ORDER BY `time` DESC LIMIT $start, $set[p_str]");
